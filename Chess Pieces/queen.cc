...
vector<pair<int, int>> Queen::possibleCaptures(const pair<int, int> &posn, Board &board, const bool secondcall){
    // Define variables
    ...
    // Iterate through all captures a queen can make
    for(const pair<int, int> &dir : directions){
        posn2 = posn; // Reset the position of the piece
...
vector<pair<int, int>> Queen::possibleChecks(const pair<int, int> &posn, Board &board){
    // Define variables
    ...
    // Iterate through possiblemoves of queen one by one
