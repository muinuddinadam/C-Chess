...
vector<pair<int, int>> Bishop::possibleCaptures(const pair<int, int> &posn, Board &board, const bool secondcall){
    // Define variables
    ...
    // Iterate through all captures a bishop can make
    for(const pair<int, int> &dir : directions){
        posn2 = posn; // Reset the position of the piece
...
vector<pair<int, int>> Bishop::possibleChecks(const pair<int, int> &posn, Board &board){
    // Define variables
    ...
    // Iterate through possiblemoves of bishop one by one
